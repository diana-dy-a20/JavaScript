...
if (confirm('Start?')) {
	...
	document.write(`Загальна вартість усіх товарів складає - ${getTotalSumPrice(dataList)}.<br>`)

	// 2)Знайти кількість товарів, у яких ціна зменшилась (price < old_price).

	function getCountProductPriceHasDecreased(arr) {
		let count = 0
		for (const product of arr) {
			product.price < product.old_price ? count ++ : count
		}
		return count
	}
	document.write(`Кількість товарів, у яких ціна зменшилась - ${getCountProductPriceHasDecreased(dataList)}<br>`)

	// 3) Товари, які доступні (sell_status:"available")

	function getFilterProduct(arr) {
		return arr.filter((product) => product.sell_status === 'available')
	}
	let filterProductList = getFilterProduct(dataList)
	...
	function getListObjectAvailable(arr) {
	...
	console.log(getListObjectAvailable(filterProductList))
}
